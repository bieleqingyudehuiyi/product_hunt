...
from django.contrib.auth.models import User  # django的用户注册的功能
...
def login(request):
    if request.method =='GET':
        ...
        user = auth.authenticate(username=user_name,password=pass_word)  #authenticate鉴定，返回None和True。对于user_name和pass_word进行鉴定
        if user is None:
